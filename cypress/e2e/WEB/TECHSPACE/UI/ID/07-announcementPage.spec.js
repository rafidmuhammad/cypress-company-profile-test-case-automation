...
describe('When testing the announcement page,', () => {
    ...
    it('should user shown winner and finalistt on announcement page', () => {
        ...
        cy.wait('@winnerStartupTrack').then(xhr => {
            onAnnouncementPage.checkWinnerTitle('id')
            onAnnouncementPage.checkYearSelect()
            onAnnouncementPage.checkWinnerTab()
            onAnnouncementPage.checkWinnerCard(0, xhr.response.body.data[0].attributes.name, xhr.response.body.data[0].attributes.description, "id")
            onAnnouncementPage.checkWinnerCard(1, xhr.response.body.data[1].attributes.name, xhr.response.body.data[1].attributes.description, "id")
            onAnnouncementPage.checkWinnerCard(2, xhr.response.body.data[2].attributes.name, xhr.response.body.data[2].attributes.description, "id")
        })

        onAnnouncementPage.clickFinalistStartUpTab()

        cy.wait('@finalisttStartupTrack').then(xhr => {
            onAnnouncementPage.checkFinalisttTitle('id')
            onAnnouncementPage.checkFinalistTab()
            onAnnouncementPage.checkFinalisttCard(0, xhr.response.body.data[0].attributes.name, xhr.response.body.data[0].attributes.description, "id")
            onAnnouncementPage.checkFinalisttCard(1, xhr.response.body.data[1].attributes.name, xhr.response.body.data[1].attributes.description, "id")
            onAnnouncementPage.checkFinalisttCard(2, xhr.response.body.data[2].attributes.name, xhr.response.body.data[2].attributes.description, "id")
        })
    })
    ...
    it('should in tahun lain menampilkan finalist and winner', () => {
        ...
        cy.wait(['@isWinner', '@yearList', '@winner2023', '@finalistt2023']).spread(
            (isWinner, yearList, winner, finalistt) => {
                onAnnouncementPage.checkWinnerCard(0, winner.response.body.data[0].attributes.name, winner.response.body.data[0].attributes.description, "id")
                onAnnouncementPage.checkFinalisttCard(0, finalistt.response.body.data[0].attributes.name, finalistt.response.body.data[0].attributes.description, "id")
            }
        )
    ...
    it('should user shown bagian Informasi More Lanjut', () => {
        onAnnouncementPage.checkForMoreInformation('id')
    })
    ...
    it('should hanya shown finalist when not yet ada winner', () => {
        ...
        cy.wait('@winner2024').then((winner) => {
            cy.wait('@finalistt2024').then((finalistt) => {
                ...
                onAnnouncementPage.checkFinalistTab()
                onAnnouncementPage.checkFinalisttCard(0, finalistt.response.body.data[0].attributes.name, finalistt.response.body.data[0].attributes.description, "id")
                onAnnouncementPage.checkFinalisttCard(1, finalistt.response.body.data[1].attributes.name, finalistt.response.body.data[1].attributes.description, "id")
                onAnnouncementPage.checkFinalisttCard(2, finalistt.response.body.data[2].attributes.name, finalistt.response.body.data[2].attributes.description, "id")
            })
        })
    ...
    it('should user shown winner and finalistt in tab lain', () => {
        ...
        cy.wait('@winnerStudentTrack').its('response.body').then(body => {
            cy.wrap(body.data).each((winner, index) => {
                onAnnouncementPage.checkWinnerCard(index, winner.attributes.name, winner.attributes.description, "id")
            })
        })
        ...
        cy.wait('@finalisttStudentTrack').its('response.body').then(body => {
            ...
            cy.wrap(body.data).each((finalistt, index) => {
                onAnnouncementPage.checkFinalisttCard(index, finalistt.attributes.name, finalistt.attributes.description, "id")
            })
        })
